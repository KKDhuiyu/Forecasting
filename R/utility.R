...
projection.scoring <- function(project, forecast, model) {
  ...
  if (forecast == "XGBoost") {
    ...
    #Base case project period 1
    x <- exp(predict(model, data.matrix(split_proj[[1]][,feature.names]))) - 1000
    x[x < 0 ] <- 0
    split_proj[[1]]$predict_xgb <- x
    #later periods
    for(i in 2:length(split_proj)){
      split_proj[[i]][,c(lag_list[-1])] <- split_proj[[i-1]][,c(lag_list[-length(lag_list)])]
      split_proj[[i]][,c(lag_list[1])] <- split_proj[[i-1]]$predict_xgb
  ...
  if (forecast == "RF") {
    ...
    #Base case project period 1
    x <- predict(model, data.matrix(split_proj[[1]][,feature.names]))
    x[x < 0 ] <- 0
    split_proj[[1]]$predict_rf <- x
    #later periods
    for(i in 2:length(split_proj)){
      split_proj[[i]][,c(lag_list[-1])] <- split_proj[[i-1]][,c(lag_list[-length(lag_list)])]
      split_proj[[i]][,c(lag_list[1])] <- split_proj[[i-1]]$predict_rf
      
      x <- predict(model, data.matrix(split_proj[[i]][,feature.names]))
      x[x < 0 ] <- 0
...
#     # Start Projecting
#     split_proj <- split.data.frame(project, project$t)
#     if (length(split_proj) > length(lag_list)) {
#       for (i in 1:(length(lag_list))) {
#         x <- exp(predict(model, data.matrix(split_proj[[i]][,feature.names]))) - 1000
#         x[x < 0 ] <- 0
#         split_proj[[i]]$predict_xgb <- x
# 
#         if (i != length(lag_list)) {
#           for (j in (i + 1):(length(lag_list))) {
#             split_proj[[j]][,lag_list[j - i]] <- x
#           }
#         }
#       }
# 
#       for (k in 2:ceiling(length(split_proj)/length(lag_list))) {
#         start <- (k - 1) * length(lag_list) + 1
# 
#         if (k * length(lag_list) <= length(split_proj)) {
#           end <- k * length(lag_list)
#         } else {
#           end <- length(split_proj)
#         }
# 
#         for (i in start:end) {
#           for (j in 2:length(lag_list)) {
#             split_proj[[i]][,lag_list[j]] <- split_proj[[i - 1]][,lag_list[j - 1]]
#           }
#           split_proj[[i]][,lag_list[1]] <- split_proj[[i - 1]]$predict_xgb
# 
#           x <- exp(predict(model, data.matrix(split_proj[[i]][,feature.names]))) - 1000
#           x[x < 0 ] <- 0
#           split_proj[[i]]$predict_xgb <- x
...
#       }
# 
#       for (k in 2:round(length(split_proj)/length(lag_list))) {
#         start <- (k - 1) * length(lag_list) + 1
# 
#         if (k * length(lag_list) <= length(split_proj)) {
#           end <- k * length(lag_list)
...
#             split_proj[[i]][,lag_list[j]] <- split_proj[[i - 1]][,lag_list[j - 1]]
#           }
#           split_proj[[i]][,lag_list[1]] <- split_proj[[i - 1]]$predict_rf
# 
#           x <- exp(predict(model, data.matrix(split_proj[[i]][,feature.names]))) - 1000
#           x[x < 0 ] <- 0
#           split_proj[[i]]$predict_rf <- x
